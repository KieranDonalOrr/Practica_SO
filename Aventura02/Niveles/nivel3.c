...
int imprimir_prompt()
{
    ...
    */
    ...
    printf("| |\\/| | | '_ \\| |  \\___ \\| '_ \\ / _ \\ | |\n");
    printf("| |  | | | | | | |  ____) | | | |  __/ | |\n");
    printf("|_|  |_|_|_| |_|_| |_____/|_| |_|\\___|_|_|  Nivel_03\n");
    printf("\n");
...
int internal_source(char **args)
{
    ...
    if (args[1] == NULL)
    {
        fprintf(stderr, "ERROR SINTAXIS - Uso: source <nombre fichero>\n");
        return EXIT_SUCCESS;
    }
...
int internal_fork(char **args)
{
    ...
    else if (childpid == 0)
    {
        ...
        fprintf(stderr, "[execute_line()→ PID hijo: %ld]\n", (long)getpid());
    ...
    {
        fprintf(stderr, "[execute_line()→ PID padre: %ld]\n", (long)getpid());

        wait(&status);

        fprintf(stderr, "[execute_line()→ Proceso hijo %d finalizado con estado: %d]\n", childpid, WEXITSTATUS(status));
