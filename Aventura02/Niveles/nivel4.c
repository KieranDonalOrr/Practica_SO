...
int internal_fork(char **args)
{
    ...
    if (childpid == 0)
    {
        ...
        //Reseteamos la señal SIGCHLD, asociandole la accion por defecto SIG_DFL
        signal(SIGCHLD, SIG_DFL);
        //Imprimimos hijo
        printf("Soy el hijo con pid = %ld\n", (long)getpid());
        //Guardamos en cmd el comando
        for (int i = 0; args[i] != NULL; i++)
        {
    ...
    //Hay un proceso en foreground
    else
    {
        //Pid del proceso en foreground
        jobs_list[0].pid = childpid;
        //Imprimimos padre
        printf("Soy el padre con pid = %ld\n", (long)getpid());
        //Mientras se este ejecutando el proceso en foreground
